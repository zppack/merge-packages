...
program.version(pkg.version, '-v, --version')
  ...
  .action((pkgs, opts) => {
    const pkgFiles = pkgs.length ? validateArguments(pkgs) : glob.sync('package*(.*).json', {
      dot: true,
      cwd: path.resolve('./'),
      ignore: 'package.merged.json',
      nodir: true,
      realpath: true,
    });
...
function validateArguments(pkgs) {
  return pkgs.reduce((res, pkgItem) => {
    if (fse.existsSync(pkgItem)) {
      const stat = fse.lstatSync(pkgItem);
      if (stat.isFile()) {
        return res.concat(pkgItem);
      }
      if (stat.isDirectory(pkgItem)) {
        return res.concat(glob.sync('package*(.*).json', {
          dot: true,
          cwd: path.resolve(pkgItem),
          ignore: 'package.merged.json',
          nodir: true,
          realpath: true,
        }));
